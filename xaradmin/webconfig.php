...
function sharecontent_admin_webconfig()
{
    ...
    foreach($data['websites'] as $key =>$website)
    {
        ...
        $sitelist[] = array('id'=>$key, 'name'=>'<a href="'.$website['homeurl'].'" title="'.xarML('submit to').' '.$website['title'].'"><img src="'.$website['image'].'" title="'.$website['title'].'" alt="'.$website['title'].'" >'.$website['title'].'</a>');
    }
    $data['value'] = $value;
